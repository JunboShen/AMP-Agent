...
def main() -> None:
    ...
    simple_message = f"""
Hi! I have a FASTA of small ORFs from human skin, oral, gut, lung, and vagina metagenomes.
Please screen anti-fungal peptide candidates (no optimization) and return the top {args.top_k}.
Use a balanced, safety-aware ranking: keep toxin/hemolysis rates low, but do not force all results to be non-toxin/non-hemolytic.
...
Context:
- Input FASTA contains small ORFs from human skin, oral, gut, lung, and vagina metagenomes.
...
- Screen the FASTA for anti-fungal peptides (no optimization), keep the top {top_k_value},
  with an emphasis on novel AFPs and balanced safety-aware ranking.
- Keep toxin/hemolysis rates low, but do NOT enforce a hard all-non-toxin/all-non-hemolytic requirement.
- Integrate potency, safety, and novelty holistically in composite ranking.
