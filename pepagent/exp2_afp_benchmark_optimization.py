...
def main() -> None:
    ...
    simple_message = f"""
Hi! I have a CSV of safe Antifungal Peptides (AFPs) candidates from screening (small ORFs from human skin/oral/lung/gut/vagina).
...
""".strip()
    ...
    detailed_message = f"""
INITIATING SAFE-TO-USE AFP OPTIMIZATION PROTOCOL (BENCHMARK, TEXTGRAD PREFERRED).

Context:
- Input CSV contains the top {args.top_k} safe Antifungal Peptides (AFPs) candidates from screening.
- Sequences are small ORFs from human skin/oral/lung/gut/vagina metagenomes.
 - Input size: {total_rows if total_rows is not None else "unknown"} rows.
 - Requested selection sizes: top_k={args.top_k}, optimize_top={args.optimize_top}.
...
- Keep sequence lengths within proper ranges for AFPs.
